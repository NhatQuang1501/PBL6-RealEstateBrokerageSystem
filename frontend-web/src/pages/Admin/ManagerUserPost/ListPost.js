      ...
      const fetchPosts = async () => {
        try {
          ...
          if (Array.isArray(response.data)) {
            const approvedPosts = response.data.filter(post => post.status === 'Đã duyệt');
            setPosts(approvedPosts);
          } else if (response.data && Array.isArray(response.data.results)) {
            const approvedPosts = response.data.results.filter(post => post.status === 'Đã duyệt');
            setPosts(approvedPosts);
          } else {
            ...
            <div className="flex flex-row px-4 py-2">
              ...
              <div className="flex flex-col justify-between w-[55%] h-[55%] border-2 border-[#DCDCDC] p-4 rounded-lg">
                ...
                <div className="w-full flex items-center justify-center gap-8 mt-5">
                  ...
                  <button className="px-5 py-2 text-[#3CA9F9] border border-[#3CA9F9] rounded-lg">Xóa bài đăng</button>
                </div>
              </div>
            </div>
          </div>

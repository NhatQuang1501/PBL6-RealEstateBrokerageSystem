...
class ChartHelper():
    ...
    def get_new_users(self, period):
        ...
        return User.objects.filter(date_joined__range=(start_date, end_date)).annotate(period=trunc_func('date_joined')).values('period').annotate(count=Count('user_id')).order_by('period')
    ...
    def get_most_interacted_post(self):
        return Post.objects.annotate(total_interactions=Count('reactions') + Count('comments') + Count('views')).order_by('-total_interactions').first()
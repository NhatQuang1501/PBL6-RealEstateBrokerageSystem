    ...
    def to_internal_value(self, data):
        ...
        if not reported_user or not reportee:
            raise serializers.ValidationError(
                "Reported user ID and reportee ID are required"
            )
        ...
        if post or comment:
            if post:
                report = Report.objects.filter(
                    reported_user=reported_user, reportee=reportee, post=post
                )
            else:
                report = Report.objects.filter(
                    reported_user=reported_user, reportee=reportee, comment=comment
                )
            if report.exists():
